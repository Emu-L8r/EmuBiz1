...
class SingleSourceOfTruthTest : BaseUnitTest() {
    ...
    fun `rule2_RevenueFilter - revenue reflects only paid invoices`() = runTest {
        ...
        every { dao.observeMTDRevenue(businessId) } returns flowOf(inv2Amount)
        every { dao.observeYTDRevenue(businessId) } returns flowOf(inv2Amount)
        every { dao.observeWeeklyRevenue(businessId) } returns flowOf(inv2Amount)
    ...
    fun `rule4_SingleSource - revenue and payment repos show consistent collected total`() = runTest {
        ...
        every { dao.observeMTDRevenue(businessId) } returns flowOf(totalCollected)
        every { dao.observeYTDRevenue(businessId) } returns flowOf(totalCollected)
        every { dao.observeWeeklyRevenue(businessId) } returns flowOf(totalCollected)
