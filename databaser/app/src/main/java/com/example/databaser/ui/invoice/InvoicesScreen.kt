...
@Composable
fun InvoicesScreen(navController: NavController, viewModel: InvoicesViewModel = hiltViewModel()) {
    ...
    Scaffold(
        ...
        floatingActionButton = {
            FloatingActionButton(onClick = { navController.navigate("${Routes.SELECT_CUSTOMER}/invoice") }) {
            ...
            items(invoices) { invoice ->
                Card(
                    modifier = Modifier
                        .padding(8.dp)
                        .fillMaxWidth()
                        .clickable { navController.navigate("${Routes.INVOICE_DETAILS}/${invoice.customerId}/${invoice.id}") }
                ) {
                    Text(
