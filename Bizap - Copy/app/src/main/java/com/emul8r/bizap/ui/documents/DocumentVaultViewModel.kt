        ...
        documentRepository.getAllDocuments()
            .combine(_searchTerm) { documents, term ->
                documents
                    ...
                    .mapNotNull { doc ->
                        invoiceRepository.getInvoiceWithItemsById(doc.relatedInvoiceId).first()?.let { invoiceWithItems ->
                            if (invoiceWithItems.invoice.customerName.contains(term, ignoreCase = true) ||
                                invoiceWithItems.invoice.invoiceId.toString().contains(term)) {
                                DocumentVaultItem(
                    ...
                    .groupBy { monthYearFormat.format(Date(it.invoice.date)) }
            }
