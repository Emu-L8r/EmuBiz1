...
object PdfGenerator {
    ...
    suspend fun createInvoicePdf(context: Context, invoice: InvoiceWithCustomerAndLineItems, customerName: String, businessInfo: BusinessInfo, isQuote: Boolean, invoiceViewModel: InvoiceViewModel): Uri? {
        val pageHeight = 1120
        val pagewidth = 792
        ...
        yPosition = drawHeaderSubheader(canvas, invoice, titlePaint, textPaint, pagewidth, yPosition)
        ...
        // Draw Photos
        yPosition = drawPhotosSection(context, canvas, invoice, yPosition, pagewidth)

        // Draw Notes
        drawFooter(canvas, businessInfo, headingPaint, textPaint, pagewidth, yPosition)
    ...
    private fun drawHeaderSubheader(canvas: Canvas, invoice: InvoiceWithCustomerAndLineItems, titlePaint: TextPaint, textPaint: TextPaint, pagewidth: Int, yPosition: Float): Float {
        var currentY = yPosition
        invoice.invoice.header?.takeIf { it.isNotBlank() }?.let {
            val headerLayout = StaticLayout.Builder.obtain(it, 0, it.length, titlePaint.apply{textSize=18f}, pagewidth - 80).build()
            canvas.save()
        ...
        invoice.invoice.subHeader?.takeIf { it.isNotBlank() }?.let {
            val subHeaderLayout = StaticLayout.Builder.obtain(it, 0, it.length, textPaint, pagewidth - 80).build()
            canvas.save()
            canvas.translate(40f, currentY)
    ...
    private suspend fun drawPhotosSection(context: Context, canvas: Canvas, invoice: InvoiceWithCustomerAndLineItems, yPosition: Float, pagewidth: Int): Float {
        var currentY = yPosition
        val photos = invoice.invoice.photoUris
    ...
    private fun drawFooter(canvas: Canvas, businessInfo: BusinessInfo, headingPaint: TextPaint, textPaint: TextPaint, pagewidth: Int, yPosition: Float) {
        var footerY = maxOf(yPosition, 900f)

        businessInfo.generalNotes?.takeIf { it.isNotBlank() }?.let {
            canvas.drawText("Notes", 40f, footerY, headingPaint)
            footerY += 20
            val notesLayout = StaticLayout.Builder.obtain(it, 0, it.length, textPaint, pagewidth / 2).build()
            canvas.save()
