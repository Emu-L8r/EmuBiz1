...
fun EditInvoiceScreen(
    ...
    val predefinedLineItems by predefinedLineItemViewModel.allPredefinedLineItems.collectAsState()
    val coroutineScope = rememberCoroutineScope()

    val currentInvoiceWithLineItems = invoiceWithLineItems

    if (currentInvoiceWithLineItems == null) {
        Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            CircularProgressIndicator()
        }
    } else {
        val customer by customerViewModel.getCustomerById(currentInvoiceWithLineItems.invoice.customerId).collectAsState(initial = null)
        ...
        var header by rememberSaveable { mutableStateOf(currentInvoiceWithLineItems.invoice.header ?: "") }
        var subHeader by rememberSaveable { mutableStateOf(currentInvoiceWithLineItems.invoice.subHeader ?: "") }
        ...
        var photoUris by remember { mutableStateOf(currentInvoiceWithLineItems.invoice.photoUris.map { it.toUri() }) }
        ...
        if (customer == null) {
            ...
            Scaffold(
                topBar = {
                    TopAppBar(
                        title = { Text(stringResource(id = R.string.edit_invoice)) },
                ...
                Column(modifier = Modifier.padding(padding).padding(16.dp)) {
                    ...
                    Card(elevation = CardDefaults.cardElevation(defaultElevation = 4.dp)) {
                        Row(modifier = Modifier.fillMaxWidth().padding(16.dp), horizontalArrangement = Arrangement.SpaceBetween) {
                            Column(modifier = Modifier.weight(1f)) {
                                Text(stringResource(id = R.string.invoice_from), style = MaterialTheme.typography.titleMedium)
                    ...
                    Card(elevation = CardDefaults.cardElevation(defaultElevation = 4.dp)) {
                        Column(modifier = Modifier.padding(16.dp)) {
                            Text(stringResource(id = R.string.invoice_details), style = MaterialTheme.typography.titleMedium)
                            Spacer(modifier = Modifier.height(8.dp))
                            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center) {
                                Column(horizontalAlignment = Alignment.CenterHorizontally) {
                                    Text(stringResource(id = R.string.invoice_number, currentInvoiceWithLineItems.invoice.invoiceNumber), style = MaterialTheme.typography.titleMedium)
                    ...
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceEvenly) {
                        Button(onClick = {
                            coroutineScope.launch {
                                ...
                                invoiceViewModel.updateInvoice(currentInvoiceWithLineItems.invoice.copy(
                                    header = header,
                                    subHeader = subHeader,
                                    date = selectedDate,
